...
def test_user_creds():
    ...
    return {"email": f"test_user_{uuid.uuid4().hex[:8]}@pronokif.com", "password": "testpass123"}
...
class TestAuthentication:
    ...
    def test_register_new_user(self, api_client):
        ...
        response = api_client.post(
            f"{BASE_URL}/api/auth/register", json={"email": unique_email, "password": "testpass123"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_login_existing_user(self, api_client):
        ...
        response = api_client.post(
            f"{BASE_URL}/api/auth/login", json={"email": "test@pronokif.com", "password": "test123"}
        )
        assert response.status_code == 200
        data = response.json()
        assert "access_token" in data
    ...
    def test_login_invalid_credentials(self, api_client):
        ...
        response = api_client.post(
            f"{BASE_URL}/api/auth/login", json={"email": "test@pronokif.com", "password": "wrongpassword"}
        )
...
class TestLeagueManagement:
    ...
    def test_create_league(self, authenticated_client):
        ...
        response = authenticated_client.post(f"{BASE_URL}/api/leagues", json={"name": league_name})
        assert response.status_code == 200
    ...
    def test_join_league_with_code(self, api_client):
        ...
        join_response = api_client.post(f"{BASE_URL}/api/leagues/join", json={"code": "CU15GA"}, headers=headers)
        # Should succeed or fail with "already a member"
        assert join_response.status_code in [200, 400]
        if join_response.status_code == 200:
            print("✅ User joined league with code CU15GA")
        else:
            print("✅ User already member or league not found")
...
class TestPredictionsWithTop10:
    ...
    def test_prediction_submission_format(self):
        ...
        if response.status_code == 200:
            ...
            print("✅ Prediction submitted with Top 10 format")
        elif response.status_code == 400:
            error = response.json()
            if "closed" in error.get("detail", "").lower():
                print("✅ Predictions closed (expected behavior)")
            else:
...
class TestSprintWeekend:
    ...
    def test_china_gp_is_sprint_weekend(self, api_client):
        ...
        print("✅ China GP correctly marked as Sprint Weekend")
    ...
    def test_regular_race_not_sprint(self, api_client):
        ...
        print("✅ Australia GP correctly marked as non-Sprint")
...
class TestAdminEndpoints:
    ...
    def test_results_submission_format(self):
        ...
        if response.status_code == 200:
            print("✅ Results submitted with Top 10 format")
        elif response.status_code == 403:
