...
class TestRaceConfiguration:
    ...
    def test_china_is_sprint_weekend(self, api_session):
        ...
        print(f"✓ china-2026 is correctly configured as sprint weekend")
    ...
    def test_japan_is_classic_weekend(self, api_session):
        ...
        assert data.get("sprint_quali_date") is None or data["sprint_quali_date"] == "None", "Sprint quali date should be None"
        assert data.get("sprint_race_date") is None or data["sprint_race_date"] == "None", "Sprint race date should be None"
        print(f"✓ japan-2026 is correctly configured as classic weekend")
...
class TestSprintPredictionValidation:
    ...
    def test_sprint_weekend_requires_sprint_quali_pole(self, api_session, auth_headers):
        ...
        assert "sprint" in error_detail.lower() or "pole" in error_detail.lower(), f"Error should mention sprint pole requirement: {error_detail}"
    ...
    def test_sprint_weekend_requires_sprint_race_winner(self, api_session, auth_headers):
        ...
        error_detail = response.json().get("detail", "")
        assert "sprint" in error_detail.lower() or "winner" in error_detail.lower(), f"Error should mention sprint winner requirement: {error_detail}"
    ...
    def test_sprint_weekend_requires_sprint_quali_top10(self, api_session, auth_headers):
        ...
        error_detail = response.json().get("detail", "")
        assert "sprint" in error_detail.lower() and "top" in error_detail.lower(), f"Error should mention sprint top 10: {error_detail}"
    ...
    def test_sprint_weekend_requires_sprint_race_top10(self, api_session, auth_headers):
        ...
        error_detail = response.json().get("detail", "")
        assert "sprint" in error_detail.lower() and "top" in error_detail.lower(), f"Error should mention sprint race top 10: {error_detail}"
...
class TestClassicPrediction:
    ...
    def test_classic_weekend_prediction_success(self, api_session, auth_headers):
        ...
        if response.status_code == 200:
            ...
            print(f"✓ Classic weekend prediction created successfully")
    ...
    def test_classic_weekend_does_not_require_sprint_fields(self, api_session, auth_headers):
        ...
        if response.status_code == 200:
            print(f"✓ Classic weekend accepts prediction without sprint fields")
...
class TestSprintPredictionSuccess:
    ...
    def test_sprint_weekend_full_prediction(self, api_session, auth_headers):
        ...
        if response.status_code == 200:
            ...
            print(f"✓ Sprint weekend full prediction created with all 8+1 fields")
        elif response.status_code == 400 and "fermés" in response.json().get("detail", "").lower():
    ...
    def test_get_prediction_includes_sprint_fields(self, api_session, auth_headers):
        ...
        if response.status_code == 200 and response.json():
            ...
            if data.get("sprint_quali_top10"):
                print(f"✓ Prediction response includes sprint_quali_top10")
            if data.get("sprint_race_top10"):
                print(f"✓ Prediction response includes sprint_race_top10")
            print(f"✓ GET prediction returns proper response structure")
        else:
            print(f"No existing prediction for miami-2026 (OK - may not have submitted yet)")
...
class TestPredictionModel:
    ...
    def test_quali_top10_must_have_10_drivers(self, api_session, auth_headers):
        ...
        print(f"✓ Validation correctly rejects incomplete quali_top10")
    ...
    def test_race_top10_must_have_10_drivers(self, api_session, auth_headers):
        ...
        print(f"✓ Validation correctly rejects incomplete race_top10")
