...
class TestSetup:
    ...
    def admin_token(self, session):
        ...
        # Try to login first
        login_res = session.post(f"{BASE_URL}/api/auth/login", json={"email": ADMIN_EMAIL, "password": ADMIN_PASSWORD})

        if login_res.status_code == 200:
            return login_res.json()["access_token"]

        # Register admin if not exists
        reg_res = session.post(f"{BASE_URL}/api/auth/register", json={"email": ADMIN_EMAIL, "password": ADMIN_PASSWORD})
    ...
    def regular_user_token(self, session):
        ...
        # Register new user
        reg_res = session.post(f"{BASE_URL}/api/auth/register", json={"email": TEST_EMAIL, "password": TEST_PASSWORD})

        if reg_res.status_code in [200, 201]:
            return reg_res.json()["access_token"]

        # Try login if already exists
        login_res = session.post(f"{BASE_URL}/api/auth/login", json={"email": TEST_EMAIL, "password": TEST_PASSWORD})
...
class TestFeedbackSubmission(TestSetup):
    ...
    def test_submit_feedback_unauthorized(self, session):
        ...
        response = session.post(f"{BASE_URL}/api/feedback", json={"category": "bug", "message": "Test message"})
...
class TestAdminFeedback(TestSetup):
    ...
    def test_admin_get_all_feedback(self, session, admin_token):
        ...
        response = session.get(f"{BASE_URL}/api/admin/feedback", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_regular_user_cannot_access_admin_feedback(self, session, regular_user_token):
        ...
        response = session.get(
            f"{BASE_URL}/api/admin/feedback", headers={"Authorization": f"Bearer {regular_user_token}"}
        )
    ...
    def test_admin_mark_feedback_read(self, session, admin_token):
        ...
        list_res = session.get(f"{BASE_URL}/api/admin/feedback", headers={"Authorization": f"Bearer {admin_token}"})

        if list_res.status_code == 200 and len(list_res.json()) > 0:
            feedback_id = list_res.json()[0]["id"]

            response = session.put(
                f"{BASE_URL}/api/admin/feedback/{feedback_id}/read", headers={"Authorization": f"Bearer {admin_token}"}
            )
            assert response.status_code == 200
...
class TestNotificationCreation(TestSetup):
    ...
    def test_admin_create_notification_invalid_type(self, session, admin_token):
        ...
        response = session.post(
            f"{BASE_URL}/api/admin/notifications",
            json={"title": "Test", "message": "Test message", "type": "invalid_type"},
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 400
...
class TestUserNotifications(TestSetup):
    ...
    def test_get_notifications(self, session, regular_user_token):
        ...
        response = session.get(
            f"{BASE_URL}/api/notifications", headers={"Authorization": f"Bearer {regular_user_token}"}
        )
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
    ...
    def test_get_unread_count(self, session, regular_user_token):
        ...
        response = session.get(
            f"{BASE_URL}/api/notifications/unread-count", headers={"Authorization": f"Bearer {regular_user_token}"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_mark_all_notifications_read(self, session, regular_user_token):
        ...
        response = session.put(
            f"{BASE_URL}/api/notifications/read-all", headers={"Authorization": f"Bearer {regular_user_token}"}
        )
        assert response.status_code == 200
