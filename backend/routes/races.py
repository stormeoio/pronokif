...
async def get_races():
    ...
    for race in F1_RACES_2026:
        ...
        if race.get("is_sprint"):
            ...
            race_response["sprint_quali_date"] = (
                race.get("sprint_quali_date", "") + "T" + sprint_quali_time + ":00+00:00"
            )
...
async def get_race(race_id: str) -> dict:
    ...
    for race in F1_RACES_2026:
        if race["id"] == race_id:
            now = datetime.now(UTC)
            race_date = datetime.fromisoformat(race["date"] + "T" + race.get("race_time", "15:00") + ":00+00:00")
            quali_date = datetime.fromisoformat(
                race["quali_date"] + "T" + race.get("quali_time", "14:00") + ":00+00:00"
            )
            predictions_close = get_predictions_close_time(race)
            sprint_predictions_close = get_sprint_predictions_close_time(race)
            ...
            return {
                ...
                "quali_date": quali_date.isoformat(),
                "sprint_quali_date": (
                    race.get("sprint_quali_date", "") + "T" + race.get("sprint_quali_time", "10:30") + ":00+00:00"
                )
                if race.get("is_sprint")
                else None,
                "sprint_race_date": (
                    race.get("sprint_race_date", "") + "T" + race.get("sprint_race_time", "14:00") + ":00+00:00"
                )
                if race.get("is_sprint")
                else None,
                "predictions_close_at": predictions_close.isoformat(),
                "sprint_predictions_close_at": sprint_predictions_close.isoformat()
                if sprint_predictions_close
                else None,
                "can_predict": can_predict,
                "can_predict_sprint": can_predict_sprint,
                "status": status,
        ...
        if race["id"] == race_id:
            ...
            result = {
                ...
                "sessions": {
                    ...
                    "fp3": {"date": race.get("fp3_date"), "time": race.get("fp3_time")}
                    if not race.get("is_sprint")
                    else None,
