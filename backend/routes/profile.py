...
async def get_user_streak(user: dict = Depends(get_current_user)) -> dict:
    """Return current prediction streak info for the caller."""
    return await profile_service.get_streak(user["id"])
