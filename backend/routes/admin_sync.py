...
async def sync_results_from_openf1(race_id: str, user=Depends(require_admin)):
...
async def send_reminder_notifications(_admin=Depends(require_admin)):
...
async def auto_sync_and_save_results(race_id: str, user=Depends(require_admin)):
...
async def sync_all_pending_races(_admin=Depends(require_admin)):
    """Manually trigger sync for all past races without results."""
    return await sync_service.sync_all_pending()


@router.get("/admin/sync-status")
async def get_sync_status(_admin=Depends(require_admin)):
