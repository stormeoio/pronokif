...
import { Menu, X, Shield, FileText, Mail, BookOpen, ChevronRight, Trophy, Calculator } from "lucide-react";
import {
  TutorialContent, RulesContent, ScoringContent,
  PrivacyContent, LegalContent, ContactContent
} from "./MenuSections";
...
export default function HamburgerMenu() {
  ...
  const closeMenu = () => { setIsOpen(false); setActiveSection(null); };
  ...
  return (
    <>
      ...
      {isOpen && <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50" onClick={closeMenu} />}
      ...
      <div className={`fixed top-0 left-0 h-full w-80 max-w-[85vw] bg-[#0a1628] border-r border-cyan-500/20 z-50 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-gray-700/50 flex items-center justify-between">
          ...
          <button onClick={closeMenu} className="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto h-[calc(100%-60px)] pb-24">
          {!activeSection ? (
              ...
              {menuItems.map((item) => (
                <button key={item.id} onClick={() => setActiveSection(item.id)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-body text-white group-hover:text-cyan-400 transition-colors">{item.label}</span>
                  </div>
            ...
            <div className="p-4 pb-32">
              <button onClick={() => setActiveSection(null)} className="flex items-center gap-2 text-cyan-400 hover:text-cyan-300 mb-4 font-body text-sm">
