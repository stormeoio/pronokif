...
export function BatakGame({ onSubmit, attemptsRemaining, isTraining = false }) {
  ...
  // Fetch user's leagues for sharing
          ...
          {Array.from({ length: gridSize.cols * gridSize.rows }).map((_, i) => {
            ...
            return (
              <button
                ...
                className={`h-14 sm:h-16 rounded-lg transition-all ${
                  target
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#0a1628] border-2 border-cyan-500/30 rounded-xl max-w-md w-full p-6 relative">
            ...
            <p className="font-body text-gray-400 text-sm mb-4">
              Score: <span className="text-cyan-400 font-data">{score} cibles</span> - {getResultMessage(score)}
            </p>
