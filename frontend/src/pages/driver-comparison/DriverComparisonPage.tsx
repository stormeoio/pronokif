...
import {
  ...
  ChevronDown, Check, Loader2, GitCompare, Crown, Award, TrendingUp
} from "lucide-react";
import {
    ...
    <div className="min-h-screen bg-app-main pb-24">
        ...
        <div className="max-w-2xl mx-auto p-4">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/championship")}
              className="p-2 bg-gray-800/50 rounded-full text-gray-400 hover:text-white hover:bg-gray-700/50 transition-colors" data-testid="back-button">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <DriverSelector drivers={allDrivers} excludeId={driver2Id} selectedId={driver1Id}
            onSelect={(id) => { setDriver1Id(id); setDropdown1Open(false); }}
            isOpen={dropdown1Open} setIsOpen={(v) => { setDropdown1Open(v); setDropdown2Open(false); }}
            testId="driver1-selector" />
          <DriverSelector drivers={allDrivers} excludeId={driver1Id} selectedId={driver2Id}
            onSelect={(id) => { setDriver2Id(id); setDropdown2Open(false); }}
            isOpen={dropdown2Open} setIsOpen={(v) => { setDropdown2Open(v); setDropdown1Open(false); }}
            testId="driver2-selector" />
        </div>

        <div className="flex justify-center mb-6">
          <button onClick={swapDrivers}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="card-arcade p-4">
              ...
              <div className="space-y-4">
                <ComparisonBar label="Titres mondiaux" icon={Crown} value1={stats?.world_championships?.driver1 || 0} value2={stats?.world_championships?.driver2 || 0} color1={getTeamColor(d1.team_id)} color2={getTeamColor(d2.team_id)} />
                <ComparisonBar label="Victoires" icon={Flag} value1={stats?.wins?.driver1 || 0} value2={stats?.wins?.driver2 || 0} color1={getTeamColor(d1.team_id)} color2={getTeamColor(d2.team_id)} />
                <ComparisonBar label="Podiums" icon={Medal} value1={stats?.podiums?.driver1 || 0} value2={stats?.podiums?.driver2 || 0} color1={getTeamColor(d1.team_id)} color2={getTeamColor(d2.team_id)} />
                <ComparisonBar label="Poles" icon={Zap} value1={stats?.poles?.driver1 || 0} value2={stats?.poles?.driver2 || 0} color1={getTeamColor(d1.team_id)} color2={getTeamColor(d2.team_id)} />
                <ComparisonBar label="Meilleurs tours" icon={Timer} value1={stats?.fastest_laps?.driver1 || 0} value2={stats?.fastest_laps?.driver2 || 0} color1={getTeamColor(d1.team_id)} color2={getTeamColor(d2.team_id)} />
                <ComparisonBar label="Points en carriere" icon={Hash} value1={stats?.points?.driver1 || 0} value2={stats?.points?.driver2 || 0} color1={getTeamColor(d1.team_id)} color2={getTeamColor(d2.team_id)} />
                <ComparisonBar label="Grands Prix" icon={Target} value1={stats?.entries?.driver1 || 0} value2={stats?.entries?.driver2 || 0} color1={getTeamColor(d1.team_id)} color2={getTeamColor(d2.team_id)} />
              </div>
            </div>
            ...
            <div className="card-arcade p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <EfficiencyCard label="Taux de victoire" value1={comparison.win_rate?.driver1 || 0} value2={comparison.win_rate?.driver2 || 0} driver1={d1} driver2={d2} suffix="%" />
                <EfficiencyCard label="Taux de podium" value1={comparison.podium_rate?.driver1 || 0} value2={comparison.podium_rate?.driver2 || 0} driver1={d1} driver2={d2} suffix="%" />
                <EfficiencyCard label="Taux de pole" value1={comparison.pole_rate?.driver1 || 0} value2={comparison.pole_rate?.driver2 || 0} driver1={d1} driver2={d2} suffix="%" />
                <EfficiencyCard label="Points/course" value1={comparison.points_per_race?.driver1 || 0} value2={comparison.points_per_race?.driver2 || 0} driver1={d1} driver2={d2} suffix="" />
              </div>
            </div>
          ...
          <div className="card-arcade p-8 text-center">
            ...
            <p className="font-body text-gray-400">Selectionnez deux pilotes differents pour les comparer</p>
          </div>
        )}
      </div>
    </div>
...
function DriverSelector({ drivers, excludeId, selectedId, onSelect, isOpen, setIsOpen, testId }: DriverSelectorProps) {
    ...
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)}
        ...
        <div className="flex items-center gap-2 truncate">
          {selected ? (
            <>
              <div className="w-8 h-8 rounded-full overflow-hidden border-2" style={{ borderColor: getTeamColor(selected.team_id) }}>
                <img src={selected.photo_url} alt="" className="w-full h-full object-cover" />
              </div>
              <span className="font-heading text-sm text-white truncate">{selected.last_name?.toUpperCase()}</span>
            </>
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 w-full bg-gray-900 border border-gray-700 rounded-lg shadow-xl max-h-64 overflow-y-auto">
          {drivers.filter(d => d.id !== excludeId).map(driver => (
            <button key={driver.id} onClick={() => onSelect(driver.id)}
              className={`w-full p-2 flex items-center gap-2 hover:bg-gray-800 transition-colors ${driver.id === selectedId ? 'bg-cyan-500/20' : ''}`}>
              <div className="w-8 h-8 rounded-full overflow-hidden border-2" style={{ borderColor: getTeamColor(driver.team_id) }}>
