...
import {
  Trophy, ChevronRight, Users, Star, Plus,
  MessageCircle, HelpCircle, Share2
} from "lucide-react";

export function LeaguesList({ userLeagues, user, unreadChatByLeague }) {
    ...
    <div className="card-arcade overflow-hidden">
      <div className="bg-gradient-to-r from-yellow-600/20 to-transparent p-3 border-b border-yellow-500/30">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => navigate("/league")}
          ...
          return (
            <div key={leagueItem.id}
              ...
              className={`flex items-center gap-3 p-3 rounded-xl transition-all cursor-pointer ${
                isActive ? 'bg-yellow-500/10 border border-yellow-500/30 hover:bg-yellow-500/20' : 'bg-white/5 hover:bg-white/10'
              }`}
              ...
              <div className="flex items-center gap-1">
                <button onClick={(e) => { e.stopPropagation(); navigate(`/league/${leagueItem.id}/chat`); }}
...
export function NoLeagueCTA() {
  ...
  return (
    <div className="card-gold p-6 text-center">
      ...
      <p className="font-body text-yellow-700 text-sm mb-5">Crée ou rejoins une ligue pour jouer avec tes amis</p>
      <Button onClick={() => navigate("/league")} className="btn-racing px-8 py-3">C'est parti !</Button>
    </div>
  );
}

export function HelpAdminCard({ onClick }) {
  return (
    <div onClick={onClick}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="font-body text-xs text-gray-400">Signalez un bug, faites une suggestion ou partagez votre avis</p>
        </div>
