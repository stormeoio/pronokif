...
const STATE_META_KEYS: Record<
  CardState,
  { labelKey: string; cls: string; Icon: typeof Check; accent: string }
> = {
  "needs-prediction": {
    labelKey: "race_carousel.status.to_predict",
    cls: "border-pk-red/40 bg-pk-red/15 text-pk-red",
    Icon: AlertCircle,
    accent: "var(--pk-red)",
  },
  predicted: {
    labelKey: "race_carousel.status.predicted",
    cls: "border-pk-emerald/30 bg-pk-emerald/12 text-pk-emerald",
    Icon: Check,
    accent: "#10b981",
  },
  finished: {
    labelKey: "race_carousel.status.finished",
    cls: "border-white/[0.12] bg-white/[0.06] text-pk-titane",
    Icon: Trophy,
    accent: "rgba(255,255,255,0.18)",
  },
  cancelled: {
    labelKey: "race_carousel.status.cancelled",
    cls: "border-pk-amber/25 bg-pk-amber/10 text-pk-amber",
    Icon: Ban,
    accent: "var(--pk-amber)",
  },
  locked: {
    labelKey: "race_carousel.status.closed",
    cls: "border-white/[0.12] bg-white/[0.06] text-pk-titane",
    Icon: Ban,
    accent: "rgba(255,255,255,0.18)",
  },
...
export default function RaceCarousel({ races, predictedRaceIds }: RaceCarouselProps) {
    ...
    return (
      <div className="rounded-md border border-white/[0.08] bg-pk-surface p-6 text-center">
        ...
        <p className="font-mono text-[0.6875rem] uppercase tracking-[0.1em] text-pk-titane">
          {t("race_carousel.calendar_unavailable")}
        </p>
      </div>
    );
  }
  ...
  return (
    <section aria-label={t("race_carousel.aria_calendar")}>
      {/* Header */}
      <div className="mb-2 flex items-end justify-between">
        <div>
          <h2 className="font-display text-[1rem] uppercase leading-none">
            {t("race_carousel.calendar")}
          </h2>
          {missingCount > 0 ? (
      ...
      >
        {ordered.map((race, i) => {
          ...
          const meta = STATE_META_KEYS[state];
          const { Icon } = meta;
          ...
          return (
            <button
              ...
              data-state={state}
            >
              {/* Vignette — full-bleed thumbnail + centered title overlay */}
              <div className="relative aspect-[16/10] w-full overflow-hidden">
                ...
                {/* Layered gradients for centered title readability */}
      ...
      {scrollable && (
        <input
          ...
          aria-label={t("race_carousel.aria_scroll")}
