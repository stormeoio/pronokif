...
export default function RaceCalendarPage() {
  ...
  const getFilteredRaces = () => {
    if (filter === "upcoming") {
      return races.filter((r: any) => r.status === "upcoming" || r.status === "in_progress");
    }
    if (filter === "completed") {
      return races.filter((r: any) => r.status === "finished");
    }
    return races;
  };
  ...
  return (
    <div className="min-h-screen bg-app-main pb-24" data-testid="race-calendar-page">
      {/* Header */}
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-3">
        {filteredRaces.length === 0 ? (
          ...
          filteredRaces.map((race: any, index: any) => {
            ...
            return (
              <div
                ...
                className={`card-arcade overflow-hidden transition-all ${
                  isNextRace ? "ring-2 ring-red-500/50" : ""
                }`}
              >
