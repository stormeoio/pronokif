...
export default function CustomPredictionsPage() {
  ...
  const removeChoice = (index) => { if (choices.length > 2) setChoices(choices.filter((_, i) => i !== index)); };
  const updateChoice = (index, text) => { const nc = [...choices]; nc[index].text = text; setChoices(nc); };
          ...
          {allRaces.length > 0 && (
            <div className="mt-3">
              ...
              <select
                ...
                onChange={(e) => setSelectedRace(allRaces.find(r => r.id === e.target.value))}
